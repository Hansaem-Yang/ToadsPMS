    ...
    const items: Maintenance[] = await query(
      `select a.vessel_no
         ...
         from (select d.vessel_no
                ...
                inner join [vessel] as d
                   on c.vessel_no = d.vessel_no) as a
        ...
        order by a.vessel_no, convert(varchar(10), case when a.due_date < getdate() then dateadd(day, -1, getdate()) else a.due_date end, 121), a.equip_no, a.section_code, a.plan_name`
    );
