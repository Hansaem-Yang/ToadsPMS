    ...
    const items: Maintenance[] = await query(
      `select a.vessel_no
        ...
        where a.vessel_no = @vesselNo
          and a.use_yn = 'Y'
          and b.due_date >= dateadd(wk, datediff(wk, 0, getdate()), 0) and b.due_date <= dateadd(wk, datediff(wk, 0, getdate()), 7)
          and (b.extension_date is null or (b.extension_date >= dateadd(wk, datediff(wk, 0, getdate()), 0) and b.extension_date <= dateadd(wk, datediff(wk, 0, getdate()), 7)))`,
      [
