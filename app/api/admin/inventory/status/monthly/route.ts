  ...
  try {
    ...
    const items: Inventory[] = await query(
      `select a.vessel_no
            ...
            , convert(varchar(7), getdate(), 121) as [period]
            , sum(case when d.receive_date >= dateadd(mm, datediff(mm, 0, getdate()), 0) and d.receive_date < dateadd(mm, datediff(mm, 0, getdate()) + 1, 0) then isnull(d.receive_qty, 0) else 0 end) as receive_qty
            , sum(case when e.release_date >= dateadd(mm, datediff(mm, 0, getdate()), 0) and e.release_date < dateadd(mm, datediff(mm, 0, getdate()) + 1, 0) then isnull(e.release_qty, 0) else 0 end) as release_qty
            , sum(case when f.loss_date >= dateadd(mm, datediff(mm, 0, getdate()), 0) and f.loss_date < dateadd(mm, datediff(mm, 0, getdate()) + 1, 0) then isnull(f.loss_qty, 0) else 0 end) as loss_qty
